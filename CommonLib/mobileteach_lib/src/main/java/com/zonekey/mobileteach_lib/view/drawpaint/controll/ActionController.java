...
import com.zonekey.mobileteach_lib.view.drawpaint.interf.IActionController;
        ...
        switch (pop.getCmdType()) {
            case ADD_SHAPE:     //如果是新增图形,则撤销哪一笔线条
                if (pop.getObj() instanceof LineInfo) {
                    ...
                    if (MobileTeach.CurrentApp == MobileTeachConfig.App_Teachmaster) { //智课助手的undo是撤销上一笔的逻辑...
        ...
        switch (actionLineInfo.getCmdType()) {
            case ADD_SHAPE:         //恢复线条,把线条加回去
                if (actionLineInfo.getObj() instanceof LineInfo) {
                    ...
                    if (TextUtils.isEmpty(send)) break;
                    if (MobileTeach.CurrentApp == MobileTeachConfig.App_Teachmaster) { //智课助手的恢复是恢复上一笔...
