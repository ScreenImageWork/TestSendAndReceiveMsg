...
public class CropOverlayView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawRect(Edge.LEFT.getCoordinate(),
                ...
                mBorderPaint);
        LogUtil.e("CropOverlayView","left " + Edge.LEFT.getCoordinate()
                + "top  " + Edge.TOP.getCoordinate() + "right  " + Edge.RIGHT.getCoordinate()
                + "bottom  " +  Edge.BOTTOM.getCoordinate());
        // Draws the corners
        drawCorners(canvas);
    }
    ...
    private void initCropWindow(Rect bitmapRect) {
        ...
        if (mFixAspectRatio) {
            ...
            final float horizontalPadding = 0.3f * bitmapRect.width();
            final float verticalPadding = 0.3f * bitmapRect.height();
    ...
    private int distance = 30;  //给上下左右的边界增加距离,防止误触取消 add by xu.wang
    ...
    private void onActionDown(float x, float y) {
        // add by xu.wang 判断是不是取消操作...
        if (x < Edge.LEFT.getCoordinate() - distance || x > Edge.RIGHT.getCoordinate() + distance ||
                y < Edge.TOP.getCoordinate() - distance || y > Edge.BOTTOM.getCoordinate() + distance) {
            isCancel = true;
            return;
        }
        final float left = Edge.LEFT.getCoordinate();
    ...
    private void onActionUp() {
        if (isCancel) { //如果该动作是取消操作,手指抬起时响应.
    ...
    private void onActionMove(float x, float y) {
        if (mPressedHandle == null || isCancel)     //add by xu.wang 如果点击区域外取消,则返回
            return;
    ...
    public Bitmap getCroppedImage(Bitmap mBitmap, ImageView mImageView) {
