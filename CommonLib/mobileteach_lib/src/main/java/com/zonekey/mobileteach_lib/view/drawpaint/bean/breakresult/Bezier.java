...
public class Bezier {
    ...
    public static final int IS_FIRST_BEZIER = 0;
    public static final int OHTER_BEZIER = 1;
    public static final int IS_LAST_BEZIER = 2;
    public final static AtomicInteger count = new AtomicInteger();

    public Bezier(PointF startP, PointF ctrlP, PointF endP) {
        this(startP, ctrlP, endP, 0);
    }

    public Bezier(PointF startP, PointF ctrlP, PointF endP, int strokeWidth) {
        this.startP = startP;
        this.ctrlP = ctrlP;
        this.endP = endP;
     ...
     * @Desc 打断线条的方法
     */
    public BezierResult breakBezier(PointF touchP, float touchRadius, int state) {
        ArrayList<BezierMath> mathLists = new ArrayList<>();

        RectF rectF = new RectF();      //触摸点 ,能碰撞的有效区域的矩形
        rectF.left = touchP.x - strokeWidth - touchRadius <= 0 ? 0 : touchP.x - strokeWidth - touchRadius;
        rectF.right = touchP.x + strokeWidth + touchRadius <= 0 ? 0 : touchP.x + strokeWidth + touchRadius;
        ...
        for (float rectFX = rectF.left; rectFX < rectF.right; rectFX++) {   //遍历碰撞区域内x方向的点
            float[] bezierT = PaintMathUtils.getBezierT(rectFX, startP.x, ctrlP.x, endP.x);//将rectFX 代入根据Bezier公式求T
            if (bezierT.length == 0) {
                continue;
            } else if (bezierT.length == 1) {
                BezierMath bezierMath = isAvailableBezierX(bezierT[0], rectFX, touchP, touchRadius);   //根据t判断是否是正确的触摸点坐标
                if (bezierMath != null) mathLists.add(bezierMath);
            } else if (bezierT.length == 2) {
                BezierMath bezierMath1 = isAvailableBezierX(bezierT[0], rectFX, touchP, touchRadius);  //根据t判断是否是正确的触摸点坐标
                if (bezierMath1 != null) mathLists.add(bezierMath1);
                BezierMath bezierMath2 = isAvailableBezierX(bezierT[1], rectFX, touchP, touchRadius);  //根据t判断是否是正确的触摸点坐标
        ...
        for (float reacFY = rectF.top; reacFY < rectF.bottom; reacFY++) {   //遍历碰撞区域y方向的点
            ...
            if (bezierT.length == 0) {
                ...
                BezierMath bezierMath = isAvailableBezierY(bezierT[0], reacFY, touchP, touchRadius);   //根据t判断是否是正确的触摸点坐标
                if (bezierMath != null) mathLists.add(bezierMath);
            } else if (bezierT.length == 2) {
                BezierMath bezierMath1 = isAvailableBezierY(bezierT[0], reacFY, touchP, touchRadius);   //根据t判断是否是正确的触摸点坐标
                if (bezierMath1 != null) mathLists.add(bezierMath1);
                BezierMath bezierMath2 = isAvailableBezierY(bezierT[1], reacFY, touchP, touchRadius);   //根据t判断是否是正确的触摸点坐标
                if (bezierMath2 != null) mathLists.add(bezierMath2);
            }
        }
        if (mathLists.size() <= 0) { //检测出了触碰区域内没有点在贝塞尔曲线上
            return null;
        }
    ...
    private BezierResult breakFromFoot(BezierMath bezierMath) {
        ...
        bezierResult.getList().add(new BezierPoint(startP, tempCtrlP, bezierMath.getBezierP()));
        return bezierResult;
    }
     ...
     */
    private BezierMath isAvailableBezierX(float t, float rectValue, PointF touchP, float touchRadius) {
    ...
    private BezierMath isAvailableBezierY(float t, float rectValue, PointF touchP, float touchRadius) {
        float x = PaintMathUtils.getBezierValue(t, startP.x, ctrlP.x, endP.x);
        PointF temp = new PointF(x, rectValue);
        float distance = PaintMathUtils.getDistance(touchP, temp);  //求当前点与触摸点中心的距离
        boolean hit = isHit(temp, touchRadius, startP, ctrlP, endP, strokeWidth);   //该点s否碰撞
        if (hit && (Math.abs(distance - (strokeWidth / 2 + touchRadius)) < 1) && distance > touchRadius) {
            return new BezierMath(t, temp, distance);
