package com.zonekey.mobileteach_lib.util;
...
public class PageInfo2PC {
    ...
    private boolean isSharePicMode = false;

    public PageInfo2PC() {
        udpUtil = new UdpUtil();
    }

    public void setSharePicMode(boolean isSharePicMode) {
        this.isSharePicMode = isSharePicMode;
    }
    ...
    public void setSendIps(List<String> lists) {
        ...
        mLists.clear();
        mLists.addAll(lists);
        if (MobileTeach.CurrentApp == MobileTeachConfig.App_MobileTeach_Student) {
            sendPort = MobileTeachConfig.teacher_udp_port;
        } else if (MobileTeach.CurrentApp == MobileTeachConfig.App_MobileTeach_Teacher) {
