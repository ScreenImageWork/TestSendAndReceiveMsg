...
import com.zonekey.mobileteach_lib.net.util.HeartBeatThread;
...
public class UdpServer {
    ...
    private void netForV1(byte[] m, DatagramPacket dp) {
        ...
        byte mainCmd = m[4];
        byte subCmd = m[6];
        if (isBeatCmd((short) (mainCmd & 0x00FF), (short) (subCmd & 0x00FF))) {
            return;
        }
        buff = new byte[2];
...
//        LogUtil.writeLog("UdpServer", body);
     ...
     */
    private boolean isBeatCmd(short response_main, short response_sub) {
    ...
    private void closeUdpServer() {
        ...
        showLog("UdpServer关闭");
        LogUtil.writeLog(TAG,"UdpServer close...");
    }
