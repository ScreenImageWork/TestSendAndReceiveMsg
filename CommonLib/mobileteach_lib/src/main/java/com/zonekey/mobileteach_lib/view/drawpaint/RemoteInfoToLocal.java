...
import com.zonekey.mobileteach_lib.view.drawpaint.bean.ActionLineInfo;
import com.zonekey.mobileteach_lib.view.drawpaint.bean.shape.LineInfo;
import com.zonekey.mobileteach_lib.view.drawpaint.bean.remote.LogicalType;
import com.zonekey.mobileteach_lib.view.drawpaint.bean.PageInfo;
import com.zonekey.mobileteach_lib.view.drawpaint.bean.remote.RemoteDocment;
import com.zonekey.mobileteach_lib.view.drawpaint.bean.remote.RemoteMiniBorad;
import com.zonekey.mobileteach_lib.view.drawpaint.bean.remote.RemotePage;
import com.zonekey.mobileteach_lib.view.drawpaint.bean.remote.RemotePoint;
import com.zonekey.mobileteach_lib.view.drawpaint.bean.remote.RemoteShape;
            ...
            for (int j = 0; j < shapes.size(); j++) {
                ...
                if (remoteShape.getLogicalType() == LogicalType.TianZiGe) {
                    ...
                    lineInfo.getPointLists().add(0,
                            new PointF(Float.parseFloat(startP[0]), Float.parseFloat(startP[1])));
                    lineInfo.getPointLists().add(1,
                            new PointF(Float.parseFloat(endP[0]), Float.parseFloat(endP[1])));
                    ...
                    for (int k = 0; k < points.size(); k++) {
                        ...
                        if (k == 0) {   //第一个点
                            PointF pointInfo = new PointF(
                                    Float.parseFloat(split[0]), Float.parseFloat(split[1]));
                            lineInfo.getPointLists().add(k, pointInfo);
                        } else {
                            PointF pointInfo = new PointF(
                                    Float.parseFloat(split[0]), Float.parseFloat(split[1]));
                            lineInfo.getPointLists().add(pointInfo);
                        }
                    }
                    ...
                    for (int k = 0; k < points.size(); k++) {
                        ...
                        PointF pointInfo = new PointF(x, y);
                        lineInfo.getPointLists().add(pointInfo);
                    }
                } else {
        ...
        for (int j = 0; j < pageInfo.getLineInfos().size(); j++) {
            ...
            switch (lineInfo.getType()) {
                ...
                case 0: //贝塞尔曲线
                    ...
                    int id = 0;
                    for (int k = 0; k < lineInfo.getPointLists().size(); k++) {
                        PointF pointInfo = lineInfo.getPointLists().get(k);
            ...
            PointF startPoint = lineInfo.getPointLists().get(0);
            remoteShape.setStartPoint(new StringBuffer().append(startPoint.x)
                    .append(",").append(startPoint.y).toString());
            PointF endPoint = lineInfo.getPointLists().get(lineInfo.getPointLists().size() - 1);
