...
import com.zonekey.mobileteach_lib.util.BitmapUtil;
...
public class PaintInfo2Pc {
     ...
     * @param type      线条类型
     * @param color     线条颜色
     * @param paintSize 线条宽度
     * @param downX
     * @param downY
     */
    public void sendDownInfo(String lineId, String type, int color, int paintSize, float downX, float downY) {
        if (!isSend) {
            return;
        }
        ...
        sendBody.append(type).append("|")
                ...
                .append(ColorUtil.convertColorToString(color)).append("|")
                .append(paintSize).append("|").append(downX)
                .append(",").append((downY));
     ...
     * @param endP
     */
    public void sendDownInfo(String lineId, String type, int color, int paintSize, PointF startP, PointF ctrlP, PointF endP) {
        if (!isSend) {
            return;
        }
        StringBuilder sendBody = new StringBuilder();
        ...
        sendBody.append(type).append("|")
                .append(lineId).append("|")
                .append(ColorUtil.convertColorToString(color)).append("|")
                .append(paintSize).append("|")
                .append(startP.x).append(",").append(startP.y).append(",")
                .append(ctrlP.x).append(",").append(ctrlP.y).append(",")
                .append(endP.x).append(",").append(endP.y);
        sendMsg(MobileTeachApi.PresentControl.Main_Cmd,
                MobileTeachApi.PresentControl.REQUEST_NEW_SHAPE, sendBody.toString());
    }
     ...
     * @param pointIndex 当前线条的第几个点
     */
    public void sendMoveInfo(String lineId, int pointIndex, PointF startP, PointF ctrlP, PointF endP) {
        if (!isSend) {
            return;
        }
        StringBuilder sendBody = new StringBuilder();
        sendBody.append(lineId).append("|")
                .append(pointIndex).append("|")
                .append(startP.x).append(",").append(startP.y).append(",")
                .append(ctrlP.x).append(",").append(ctrlP.y).append(",")
                .append(endP.x).append(",").append(endP.y);
        sendMsg(MobileTeachApi.PresentControl.Main_Cmd,
...
//        LogUtil.writeLog("PaintInfo2Pc", "手指移动" + sendBody.toString());
    ...
    public void setMoveShape(String lineId, int currentPoint, float mX, float mY) {
        ...
        sendBody.append(lineId).append("|")
                .append(currentPoint).append("|").
                append(mX + "," + mY);
//        LogUtil.e("PaintInfo2Pc","" + sendBody.toString());
        sendMsg(MobileTeachApi.PresentControl.Main_Cmd,
                MobileTeachApi.PresentControl.REQUEST_ADD_POINT, sendBody.toString());
    }
